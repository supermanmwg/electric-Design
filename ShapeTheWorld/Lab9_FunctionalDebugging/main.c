...
#define PF0 (GPIO_PORTF_DATA_R & 0x01)
#define PF4 ((GPIO_PORTF_DATA_R & 0x10)>>0x4)
...
int main(void){  unsigned long i,last,now,lastPF0,lastPF4;
  ...
  while(1){
		if(0 == PF4 || 0 == PF0) {
			...
			if(i<50){
				...
				Data[i] = GPIO_PORTF_DATA_R |
									PF4 << 4  | 
									PF0 ; // record PF0,PF1,PF4
				last = now;
				i++;
			}
			Delay();
		  ...
		  lastPF0 = PF0;
			...
			if(i<50&&(lastPF4 != PF4 || lastPF0 != PF0)){
				now = NVIC_ST_CURRENT_R;
				Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
